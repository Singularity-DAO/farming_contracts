...
const SDAOBondedTokenStaking = artifacts.require('SDAOBondedTokenStaking')
...
contract('SDAOBondedTokenStaking', ([alice, bob, carol, dev, minter]) => {
  before(async () => {
    const initialMint = '500000000'
    const initialMintBN = new BigNumber(initialMint).times(rewardsFactor)

    this.sdao = await SDAOToken.new('SDAO', 'SDAO', initialMintBN.toFixed(), {
      from: minter,
    })
  })
  ...
  const displayUserInfo = async (poolId, userWallet) => {
    ...
    const { amount, rewardDebt } = await this.sdaostaking.userInfo.call(
      poolId,
      userWallet,
    )
  ...
  const displayPoolInfo = async (poolId) => {
    ...
    const {
      ...
      lastRewardBlock,
      endOfEpochBlock,
    } = await this.sdaostaking.poolInfo.call(poolId)
  ...
  context('With ERC/LP token added to the field', () => {
    before(async () => {
      ...
      const initialLP2TransferBN = new BigNumber('200000').times(rewardsFactor)

      this.lp = await MockERC20.new('LPToken', 'LP', initialMintBN.toFixed(), {
        from: minter,
      })

      await this.lp.transfer(alice, initialLPTransferBN.toFixed(), {
        from: minter,
      })
      await this.lp.transfer(bob, initialLPTransferBN.toFixed(), {
        from: minter,
      })
      await this.lp.transfer(carol, initialLPTransferBN.toFixed(), {
        from: minter,
      })
      ...
      this.lp2 = await MockERC20.new(
        'LPToken2',
        'LP2',
        initialMintBN.toFixed(),
        { from: minter },
      )

      await this.lp2.transfer(alice, initialLP2TransferBN.toFixed(), {
        from: minter,
      })
      await this.lp2.transfer(bob, initialLP2TransferBN.toFixed(), {
        from: minter,
      })
      await this.lp2.transfer(carol, initialLP2TransferBN.toFixed(), {
        from: minter,
      })
    })
    ...
    it('Create staking instance and Add rewards', async () => {
      ...
      this.sdaostaking = await SDAOBondedTokenStaking.new(this.sdao.address, {
        from: minter,
      })

      // add the rewards to the Staking Contract
      await this.sdao.approve(
        this.sdaostaking.address,
        stakingRewardBN.toFixed(),
        { from: minter },
      )
      ...
      await this.sdaostaking.addRewards(stakingRewardBN.toFixed(), {
        from: minter,
      })

      // Minter balance should reduce
      assert.equal(
        new BigNumber(await this.sdao.balanceOf(minter)).toFixed(),
        new BigNumber(minterRewardTokenBal).minus(stakingRewardBN).toFixed(),
      )

      // Total Rewards Received in the contract should get incremented
      assert.equal(
        new BigNumber(await this.sdaostaking.totalRewardsReceived()).toFixed(),
        new BigNumber(totalRewardsReceived).plus(stakingRewardBN).toFixed(),
      )
    })
    ...
    it('Create epoc with One Staker for end to end operations', async () => {
      ...
      const rewardPerBlockBN = new BigNumber(rewardPerBlock).times(
        rewardsFactor,
      )
      let stakeAmount = 100
      ...
      // Create a New Pool
      await this.sdaostaking.add(
        this.lp.address,
        rewardPerBlockBN.toFixed(),
        endEpoCBlockNumber,
        { from: minter },
      )
      ...
      await this.lp.approve(
        this.sdaostaking.address,
        approveAmountBN.toFixed(),
        { from: alice },
      )
      const depositLog = await this.sdaostaking.deposit(
        poolId,
        stakeAmountBN.toFixed(),
        alice,
        { from: alice },
      )
      ...
      assert.equal(
        new BigNumber(rewards_b).plus(rewardPerBlockBN).toFixed(),
        new BigNumber(rewards_a).toFixed(),
      )

      // Deposit to Non existing Pool Should fail
      await expectRevert(
        this.sdaostaking.deposit('1001', '100', alice, { from: alice }),
        'invalid opcode',
      )
      ...
      assert.equal(
        new BigNumber(
          await this.sdaostaking.pendingRewards(poolId, alice),
        ).toFixed(),
        new BigNumber(17).times(rewardsFactor).toFixed(),
      )

      // All Negative Test Cases
      // Alice to withdraw and Harvest higher Amount then deposited - Should fail.
      await expectRevert(
        this.sdaostaking.withdrawAndHarvest(
          poolId,
          stakeAmountBN.plus(stakeAmountBN).toFixed(),
          alice,
          { from: alice },
        ),
      ...
      await expectRevert(
        this.sdaostaking.withdraw(poolId, stakeAmountBN.toFixed(), bob, {
          from: bob,
        }),
        'Invalid amount to withdraw.',
      )
      ...
      const receiptHarvestBob = await this.sdaostaking.harvest(poolId, bob, {
        from: bob,
      })
      ...
      assert.equal(
        ...
        new BigNumber(37).times(rewardsFactor).toFixed(),
      )
      ...
      const expectedReward = await this.sdaostaking.pendingRewards(
        poolId,
        alice,
      )
      ...
      await this.sdaostaking.withdrawAndHarvest(
        poolId,
        stakeAmountBN.toFixed(),
        alice,
        { from: alice },
      ...
      assert.equal(
        new BigNumber(aliceRewardsTokenBal_a).toFixed(),
        new BigNumber(aliceRewardsTokenBal_b).plus(expectedReward).toFixed(),
      )
      assert.equal(
        new BigNumber(aliceLPTokenBal_a).toFixed(),
        stakeAmountBN.plus(aliceLPTokenBal_b).toFixed(),
      )
    })
    ...
    it('Create epoc with multiple Stakers for end to end operations', async () => {
      ...
      const depositAliceLog = await this.sdaostaking.deposit(
        poolId,
        stakeAmountAliceBN.toFixed(),
        alice,
        { from: alice },
      )
      ...
      const depositBobLog = await this.sdaostaking.deposit(
        poolId,
        stakeAmountBobBN.toFixed(),
        bob,
        { from: bob },
      )
      ...
      // Advance by Block Number and compare the Rewards
      const rewardsAlice_b = await this.sdaostaking.pendingRewards(
        poolId,
        alice,
      )
      const rewardsBob_b = await this.sdaostaking.pendingRewards(poolId, bob)
      await time.advanceBlock()
      const rewardsAlice_a = await this.sdaostaking.pendingRewards(
        poolId,
        alice,
      )
      ...
      assert.equal(
        new BigNumber(rewardsAlice_b)
          .plus(new BigNumber(1).times(rewardsFactor))
          .toFixed(),
        new BigNumber(rewardsAlice_a).toFixed(),
      )

      // Alice Rewards - Should increment by 3 (300 * 4 /  400) for tokenPerBlock = 4
      //assert.equal(rewardsBob_b + 3, rewardsBob_a);
      assert.equal(
        new BigNumber(rewardsBob_b)
          .plus(new BigNumber(3).times(rewardsFactor))
          .toFixed(),
        new BigNumber(rewardsBob_a).toFixed(),
      )

      // Moved to the end of the epoc
      await time.advanceBlockTo(endEpoCBlockNumber + 10)

      const expectedRewardAlice = await this.sdaostaking.pendingRewards(
        poolId,
        alice,
      )
      const expectedRewardBob = await this.sdaostaking.pendingRewards(
        poolId,
        bob,
      )

      const calcRewardAlice =
        (depositBobLog.receipt.blockNumber -
          depositAliceLog.receipt.blockNumber) *
          rewardPerBlock +
        (endEpoCBlockNumber - depositBobLog.receipt.blockNumber) * 1
      const calcRewardBob =
        (endEpoCBlockNumber - depositBobLog.receipt.blockNumber) * 3

      assert(
        new BigNumber(expectedRewardAlice).toFixed(),
        new BigNumber(calcRewardAlice).times(rewardsFactor).toFixed(),
      )
      assert(
        new BigNumber(expectedRewardBob).toFixed(),
        new BigNumber(calcRewardBob).times(rewardsFactor).toFixed(),
      )
      ...
      assert.equal(
        new BigNumber(aliceRewardsTokenBal_a).toFixed(),
        new BigNumber(aliceRewardsTokenBal_b)
          .plus(expectedRewardAlice)
          .toFixed(),
      )
      assert.equal(
        new BigNumber(aliceLPTokenBal_a).toFixed(),
        new BigNumber(aliceLPTokenBal_b).plus(stakeAmountAliceBN).toFixed(),
      )

      assert.equal(
        new BigNumber(bobRewardsTokenBal_a).toFixed(),
        new BigNumber(bobRewardsTokenBal_b).plus(expectedRewardBob).toFixed(),
      )
      assert.equal(
        new BigNumber(bobLPTokenBal_a).toFixed(),
        new BigNumber(bobLPTokenBal_b).plus(stakeAmountBobBN).toFixed(),
      )
    ...
    it('Create the new epoc with the real production numbers - 1 ', async () => {
      ...
      const depositCarolLog = await this.sdaostaking.deposit(
        poolId,
        stakeAmountCarolBN.toFixed(),
        carol,
        { from: carol },
      )
      ...
      const expectedRewardBob_1 = await this.sdaostaking.pendingRewards(
        poolId,
        bob,
      )
      ...
      const expectedRewardCarol = await this.sdaostaking.pendingRewards(
        poolId,
        carol,
      )
      const withdrawHarvestCarolLog = await this.sdaostaking.withdrawAndHarvest(
        poolId,
        stakeAmountCarolBN.toFixed(),
        carol,
        { from: carol },
      ...
      const calcRewardsCarol = rewardPerBlockBN
        .times(
          withdrawHarvestCarolLog.receipt.blockNumber -
            depositCarolLog.receipt.blockNumber -
            1,
        )
        .times(rewardsFactor)
        .div(stakeAmountAliceBN.plus(stakeAmountBobBN).plus(stakeAmountCarolBN))
        .times(stakeAmountCarolBN)
        .div(rewardsFactor)
      ...
      const expectedRewardBob_2 = await this.sdaostaking.pendingRewards(
        poolId,
        bob,
      )
      ...
      // Bob has two time harvest & withdrawAndHarvest -- Need to come up with some other comparision for large numbers
      ...
      assert.equal(
        new BigNumber(carolLPTokenBal_a).toFixed(),
        new BigNumber(carolLPTokenBal_b).plus(stakeAmountCarolBN).toFixed(),
      )
    })
    ...
    it('Create the new epoc with the real production numbers - 2 ', async () => {
      ...
      let stakeAmountBobBN = new BigNumber(16312).times(rewardsFactor)
      let stakeAmountCarolBN = new BigNumber(19048).times(rewardsFactor)
      ...
      const withdrawCarolLog = await this.sdaostaking.withdraw(
        poolId,
        stakeAmountCarolBN.toFixed(),
        carol,
        { from: carol },
    ...
    it('Emergency Withdraw by Staker', async () => {
      ...
      assert.equal(
        new BigNumber(aliceRewardsTokenBal_a).toFixed(),
        new BigNumber(aliceRewardsTokenBal_b).toFixed(),
      )
      assert.equal(
        new BigNumber(aliceLPTokenBal_a).toFixed(),
        new BigNumber(aliceLPTokenBal_b).plus(stakeAmountAliceBN).toFixed(),
    ...
    it('Owner withdrawing tokens from the Contract ', async () => {
      ...
      const contractRewardsTokenBal_b = await this.sdao.balanceOf(
        this.sdaostaking.address,
      )

      // Owner withdraws the tokens
      await this.sdaostaking.withdrawETHAndAnyTokens(this.sdao.address, {
        from: minter,
      })

      const minterRewardsTokenBal_a = await this.sdao.balanceOf(minter)
      const contractRewardsTokenBal_a = await this.sdao.balanceOf(
        this.sdaostaking.address,
      )

      assert.equal(
        new BigNumber(minterRewardsTokenBal_a).toFixed(),
        new BigNumber(minterRewardsTokenBal_b)
          .plus(contractRewardsTokenBal_b)
          .toFixed(),
      )
    ...
    //     this.sdaostaking = await SDAOBondedTokenStaking.new(this.sdao.address, { from: minter });
