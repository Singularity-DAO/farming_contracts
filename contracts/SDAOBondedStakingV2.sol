...
contract TokenStake is Ownable {
    ...
    ERC20 public rewardtoken; //Address of reward token contract
    ...
    uint256 private maxstake = 25000 * 1e18;

    uint256 private reward = 1 * 1e18;
    ...
    modifier validStakeLimit(address staker, uint256 stakeAmount) {
        ...
        require(stakerTotalStake < maxstake,"Your total stake is too high, max is 25000 SDAO")
        // Check for Min Stake
        require(
            stakeAmount > 0 && 
    ...
    constructor(address _rewardtoken,address _token, uint256 _maxMigrationBlocks)
    public
    {
        token = ERC20(_token);
    ...
    returns(bool)
    {
        ...
        //check if reward is 30% of totalAmount

        uint256 amount =  totalAmount/100;
        uint256 minimumreward = amount.mul(30);

         // set reward to 30% minumum if below
        if(stakeAmount < minimumreward) {
            rewardAmount = minimumreward;
        }
    ...
    function claimStake(uint256 stakeMapIndex) public allowClaimStake(stakeMapIndex) {
        ...
        require(token.balanceOf(address(this)) >= stakeAmount, "Not enough balance SDAO in the contract");
        //Check  dummy reward balance 
        require(rewardtoken.balanceOf(address(this)) >= reward, "Not enough balance of rewardtoken in the contract");
        // Update the User Balance
        balances[msg.sender] = balances[msg.sender].sub(stakeAmount);

        // Call the transfer function
        require(token.transfer(msg.sender, stakeAmount), "Unable to transfer SDAO token back to the account");
        //transfer dummy reward 
        require(rewardtoken.transfer(msg.sender, reward), "Unable to transfer rewardtoken to the account");
